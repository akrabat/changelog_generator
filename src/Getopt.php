<?php
// This class is extracted from Laminas\Console 2.8.0 and placed in the App namespace.
 ...
 * - Raise exception in several cases when invalid flags or parameters
 *   are given.  Usage message is returned in the exception object.
 *
...
class Getopt
{
     ...
     *
     * @param  array $argv
     * @throws InvalidArgumentException When not given an array as parameter
     * @return self
     */
    public function addArguments($argv)
    {
        if (!is_array($argv)) {
            throw new InvalidArgumentException("Parameter #1 to addArguments should be an array");
        }
    ...
    {
        if (!is_array($argv)) {
            throw new InvalidArgumentException("Parameter #1 to setArguments should be an array");
        }
        $this->argv = $argv;
        $this->parsed = false;
        return $this;
    ...
    public function setAliases($aliasMap)
    {
        foreach ($aliasMap as $flag => $alias) {
            ...
            if (isset($this->rules[$alias]) || isset($this->ruleMap[$alias])) {
                ...
                throw new InvalidArgumentException("Option \"$o\" is being defined more than once.");
            }
     ...
     *                           false then an RuntimeException will be thrown indicating that
    ...
    protected function _setNumericOptionValue($value)
    {
        if (!$this->getoptConfig[self::CONFIG_NUMERIC_FLAGS]) {
            throw new RuntimeException("Using of numeric flags are deny by configuration");
        }

        if (empty($this->getoptConfig['numericFlagsOption'])) {
            throw new RuntimeException("Any option for handling numeric flags are specified");
        }
